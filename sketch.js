class Sketch {
  constructor(opts) {
    ...
    this.clicker = document.querySelectorAll(".project-name-btn");
  ...
  clickEvent(){
    ...
    //   this.next();
    // })
    this.clicker.forEach(item => {
      item.addEventListener("mouseenter", () => this.change(Number(item.getAttribute('data-index'))));
      // item.addEventListener('mouseleave', () => this.change(0));
    })
    document.getElementById("project-list").addEventListener("mouseleave", () => {
      this.change(0);
  ...
  change(index) {
    ...
    this.tl.to(this.material.uniforms.progress, this.duration, {
      value: 1,
      ease: Power2[this.easing],
      onComplete: () => {
        // console.log('FINISH');
